...
async function startNfcScan() {
  ...
  if (!targetDevice && !pairingToken) {
    setScanStatus(
      "Kein NFC-Reader konfiguriert. Bitte in den Geräte-Einstellungen einen Enrollment-Reader festlegen.",
      "error",
    );
    return;
  }
  ...
  const sseUrl = pairingToken ? `/api/scans/stream?token=${encodeURIComponent(pairingToken)}` : `/api/scans/stream`;
...
async function pollWriteResult(deviceId, requestId, attempts = 0) {
  ...
  try {
    const res = await fetch(
      `/api/write-result?device_id=${encodeURIComponent(deviceId)}&request_id=${encodeURIComponent(requestId)}`,
    );
    const data = await res.json();
    if (data.found) {
      if (data.success) {
...
async function manualProvisionLogin(mitgliedId) {
  try {
    ...
    if (!res.ok) {
      alert("Fehler beim Einrichten des Logins");
      return;
    }
    const data = await res.json();
  ...
  try {
    ...
    content.innerHTML = `
            <form id="member-details-form" class="form-grid">
                ...
                <div class="form-group form-group-full" style="margin-top: 16px; padding: 12px; background: var(--bg-tertiary); border-radius: 6px; border: 1px solid var(--border-color);">
                    <label style="margin-bottom: 8px; display: block;">Geräte-Berechtigungen</label>
                    <div id="permissions-container" style="margin-bottom: 12px;">
                        <span style="color: var(--text-secondary); font-size: 0.9rem;">Lade Berechtigungen...</span>
                    </div>
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <select id="permission-device-select" style="flex: 1; padding: 6px 10px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--bg-secondary); color: var(--text-primary);">
                            <option value="">Gerät auswählen...</option>
                            <option value="*">Alle Geräte (*)</option>
                        </select>
                        <button type="button" class="btn btn-sm btn-primary" onclick="addPermission(${id})">Hinzufügen</button>
                    </div>
                </div>
...
async function loadPermissions(memberId) {
  try {
    const res = await fetch(`/api/mitglieder/${memberId}/permissions`);
    if (!res.ok) {
    ...
    if (permissions.length === 0) {
      container.innerHTML =
        '<span style="color: var(--text-secondary); font-size: 0.9rem;">Keine Berechtigungen</span>';
    } else {
      container.innerHTML = permissions
        .map(
          (p) => `
        <div style="display: flex; align-items: center; gap: 8px; padding: 6px 10px; background: var(--bg-secondary); border-radius: 4px; margin-bottom: 6px;">
          <code style="background: var(--bg-tertiary); padding: 2px 6px; border-radius: 3px; font-size: 0.85rem;">${esc(p.device_id)}</code>
          <span style="color: var(--text-secondary); font-size: 0.8rem;">seit ${p.granted_at ? new Date(p.granted_at).toLocaleDateString("de-DE") : "-"}</span>
          <button type="button" class="btn btn-sm btn-secondary" style="margin-left: auto; padding: 2px 8px; font-size: 0.8rem;" onclick="deletePermission(${memberId}, ${p.id})">Entfernen</button>
        </div>
      `,
        )
        .join("");
...
async function loadDevicesForSelect() {
  try {
    ...
    const select = document.getElementById("permission-device-select");
    select.innerHTML = '<option value="">Gerät auswählen...</option><option value="*">Alle Geräte (*)</option>';
    devices.forEach((d) => {
...
async function addPermission(memberId) {
  const select = document.getElementById("permission-device-select");
...
async function deletePermission(memberId, permissionId) {
  if (!confirm("Berechtigung wirklich entfernen?")) return;
  try {
    const res = await fetch(`/api/mitglieder/${memberId}/permissions/${permissionId}`, {
      method: "DELETE",
    });
    if (res.ok) {
      await loadPermissions(memberId);
      ...
      const err = await res.json();
      alert("Fehler: " + (err.detail || "Entfernen fehlgeschlagen"));
    }
  } catch (e) {
    alert("Fehler: " + e.message);
  }
